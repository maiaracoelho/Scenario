...
if(len(sys.argv) < 5):
	...
	print_e("$ python3 " + str(sys.argv[0]) + " <SAMPLES_FILE> <time_ms> <initial_interval> <final_interval>")
	print("\nEncerrando.")
	exit()
...
initial_interval = 0
try:
    initial_interval = str(sys.argv[3])
...
not0_count = 1		# Contador de taxas nao zeradas
...
selected_samples_not0count = not0_count
...
while(i < len(samples)):
	...
	# Verifica se q quantidade atual de esparcidade do intervalo eh menor que a do historico
	if(hasUpdate):		
		rates_mean = rates_sum / (rates_count)
		if(selected_samples_not0count < not0_count):
			selected_rates_mean = rates_mean
			selected_start = i0
			selected_samples_count = rates_count
			selected_samples_not0count = not0_count
...
carac = "Intervalo com menor esparcidade: "
